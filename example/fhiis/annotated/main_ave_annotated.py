...
from .models.maskformer_predictor import TransformerPredictor
from .models.audio_net import UnetIquery
from .models.vision_net import Resnet, ResnetDilated, ResnetFC
from .models.criterion import BaseLoss
...
class NetWrapper(torch.nn.Module):
    def __init__(
        self,
        maskformer: TransformerPredictor,
        nets: Tuple[UnetIquery, ResnetDilated],
        crit: BaseLoss,
    ):
    ...
    def forward(self, batch_data, args: Args_, cur_num_mix=2):
        mag_mix: MagMix_ = batch_data["mag_mix"]
        mags: Mags_ = batch_data["mags"]
        frames: Frames_ = batch_data["frames"]
        classes: Classes_ = batch_data["classes"]
        ...
        for n in range(N):
            pred_masks[n] = self.net_maskformer.forward(
                feat_latent, feat_sound, classes[n], feat_frames[n]
            )
...
# Calculate metrics
def calc_metrics(batch_data, outputs, args:Args_, cur_num_mix):
    # meters
    ...
    for n in range(N):
        if args.log_freq:
            grid_unwarp = torch.from_numpy(
                warpgrid(
                    B, args.stft_frame // 2 + 1, pred_masks_[0].size(3), warp=False
                )
            ).to(args.device)
    ...
    for n in range(N):
        ...
        if args.binary_mask:
            pred_masks_linear[n] = (pred_masks_linear[n] > args.mask_thres).astype(
                np.float32
            )
    ...
    for j in range(B):
        ...
        if valid:
            sdr, sir, sar, _ = bss_eval_sources(
                np.asarray(gts_wav), np.asarray(preds_wav), False
            )
            sdr_mix, _, _, _ = bss_eval_sources(
                np.asarray(gts_wav), np.asarray([mix_wav[0:L] for n in range(N)]), False
            )
...
def output_visuals(vis_rows, batch_data, outputs, args:Args_, cur_num_mix):
    # fetch data and predictions
    mag_mix = batch_data["mag_mix"]
    phase_mix = batch_data["phase_mix"]
    ...
    for n in range(N):
        if args.log_freq:
            grid_unwarp = torch.from_numpy(
                warpgrid(B, args.stft_frame // 2 + 1, gt_masks_[0].size(3), warp=False)
            ).to(args.device)
            pred_masks_linear[n] = F.grid_sample(pred_masks_[n], grid_unwarp)
    ...
    for j in range(B):
        ...
        weight = magnitude2heatmap(weight_[j, 0], log=False, scale=100.0)
        ...
        for n in range(N):
            ...
            gt_wav = istft_reconstruction(
                gt_mag, phase_mix[j, 0], hop_length=args.stft_hop
            )
            pred_mag = mag_mix[j, 0] * pred_masks_linear[n][j, 0]
            preds_wav[n] = istft_reconstruction(
                pred_mag, phase_mix[j, 0], hop_length=args.stft_hop
            ...
            wavfile.write(
                os.path.join(args.vis, filename_predwav), args.audRate, preds_wav[n]
            )
...
def evaluate(netWrapper: NetWrapper, loader, history, epoch, args:Args_, cur_num_mix):
        ...
        if i % args.disp_iter == 0:
            print(
                ...
                "loss: {:.4f}".format(
                    ...
                    args.lr_sound,
                    args.lr_frame,
                    mask_optimizer.state_dict()["param_groups"][0]["lr"],
                    err.item(),
                )
            )
            fractional_epoch = epoch - 1 + 1.0 * i / args.epoch_iters
            history["train"]["epoch"].append(fractional_epoch)
            history["train"]["err"].append(loss.mean().item())
    mask_lr_scheduler.step()


def checkpoint(nets, maskformer, history, epoch, args:Args_):
    ...
    torch.save(net_sound.state_dict(), "{}/sound_{}".format(args.ckpt, suffix_latest))
    torch.save(net_frame.state_dict(), "{}/frame_{}".format(args.ckpt, suffix_latest))
    torch.save(
        maskformer.state_dict(), "{}/maskformer_{}".format(args.ckpt, suffix_latest)
    )
    ...
    if cur_sdr > args.best_sdr:
        ...
        torch.save(net_sound.state_dict(), "{}/sound_{}".format(args.ckpt, suffix_best))
        torch.save(net_frame.state_dict(), "{}/frame_{}".format(args.ckpt, suffix_best))
        torch.save(
            maskformer.state_dict(), "{}/maskformer_{}".format(args.ckpt, suffix_best)
        )


def create_optimizer(nets, args:Args_):
    (net_sound, net_frame) = nets
    param_groups = [
...
def maskformer_optimizer(model, args:Args_):
    param_dicts = [
        {"params": [p for n, p in model.named_parameters() if p.requires_grad]}
    ]
    return torch.optim.AdamW(
        param_dicts, lr=args.lr_maskformer, weight_decay=args.weight_decay_maskformer
    )
...
def adjust_learning_rate(optimizer, args:Args_):
    args.lr_sound *= 0.1
    args.lr_frame *= 0.1
...
def main(args:Args_):
    ...
    # Dataset and Loader
    dataset_train = AVEDataset(args.list_train, args.num_mix, args, split="train")
...
if __name__ == "__main__":
    # arguments
    parser = ArgParser()
    args: Args = parser.parse_train_arguments()
